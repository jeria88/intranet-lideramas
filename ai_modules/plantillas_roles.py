"""Plantillas de los asistentes IA por rol directivo.

Genéricas: no pertenecen a ningún cliente. Son conocimiento de dominio sobre
gestión escolar chilena (Ley 21.013, Código Procesal Penal, RICE, RIOHS,
Estatuto Docente, decretos 67/83/170) y se instancian para cualquier
organización con `manage.py crear_organizacion`.

Cada `prompt_*(nombre_establecimiento)` devuelve el system_instruction del rol
para esa sede; `_REGLA_URGENCIA` se antepone a todos y decide cuándo una
consulta activa denuncia obligatoria.

Extraído de `setup_all_establishments.py`, que traía hardcodeados los 8 colegios
de un cliente y su nombre dentro de cada prompt.
"""
...
def prompt_inspector(est_name):
    return f"""Eres el/la Inspector/a General de {est_name}.
...
def prompt_convivencia(est_name):
    return f"""Eres el/la Coordinador/a de Convivencia Educativa de {est_name}.
...
def prompt_director(est_name):
    return f"""Eres el/la Director/a de {est_name}.
...
def prompt_utp(est_name):
    return f"""Eres el/la Jefe/a de la Unidad Técnico Pedagógica (UTP) de {est_name}.
...
def prompt_representante(est_name):
    return f"""Eres el/la Representante Legal de {est_name}.
